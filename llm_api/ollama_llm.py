...
from ..llm_api.base_llm import BaseLLM
from ..config.llm_config import LLMConfig, LLMType
from ..llm_api.general_api_requestor import GeneralAPIRequestor
...
class OllamaLLM(BaseLLM):
    ...
    def __init__ollama__(self, config: LLMConfig):
        ...
        self.model_tool = config.model_tool
        # self.json_tool = config.json_tool
    ...
    def _const_kwargs(self, messages: list[dict], stream: bool = False, user_stop_words: list = [], use_tool: bool = False, temperature: float = None,tools:list = None) -> dict:
        # 合并默认终止符和用户提供的终止符
        # 提取system消息
        kwargs = {
            "model": self.model, 
    ...
    async def _whoami_text(self, messages: List[Dict[str, str]], timeout: int, user_stop_words: List[str]=[], use_tool: bool = False,temperature: float = None, tool_call_json: list = None) -> dict:
        """Non-streaming text completion"""
        try:
            start_time = time.time()
            self.logger.debug(f"Starting _whoami_text with timeout {timeout}")
            self.logger.debug(f"Request params: {self._const_kwargs(messages=messages, user_stop_words=user_stop_words,use_tool=use_tool,tools=tool_call_json,temperature=temperature)}")
            resp, _, _ = await self.client.arequest(
                method=self.http_method,
                url=self.suffix_url,
                params=self._const_kwargs(messages=messages, user_stop_words=user_stop_words,use_tool=use_tool,temperature=temperature,tools=tool_call_json),
