...
class PlanningAgent:
    end_flag: int = 0
    args_schema: Type[BaseModel] = PlanningAgentSchema
    ...
    async def agent_execute(self, query, chat_history=[], retrieval_flag=False):
    ...
    async def agent_execute_with_retry_async(self, query, chat_history=[], retry_times=3, retrieval_flag=False):
        for i in range(retry_times):
            status, result, chat_history = await self.agent_execute(query, chat_history=chat_history, retrieval_flag=retrieval_flag)
            if status:
                return status, result, chat_history
        return status, result, chat_history

    
    def agent_execute_with_retry(self, query, chat_history=[], retry_times=3, retrieval_flag=False):
        """
        ...
        if asyncio.get_event_loop().is_running():
            ...
            return self.agent_execute_with_retry_async(query, chat_history, retry_times, retrieval_flag)
        else:
            ...
            try:
                return loop.run_until_complete(
                    self.agent_execute_with_retry_async(query, chat_history, retry_times, retrieval_flag)
                )
            finally:
                loop.close()
    ...
    async def execute(
        ...
        status, result, chat_history = await self.agent_execute_with_retry(
            query=question,
...
if __name__ == '__main__':
    ...
    async def main():
        _, result, _ = await planning_agent.execute(
            question="你们公司地址",
            chat_history=[]
        )
        print(result)
